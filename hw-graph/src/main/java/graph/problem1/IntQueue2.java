...
public final class IntQueue2 {
    ...
    // Represents a queue as a circular ring buffer.
    // The entries in the queue = entries.
    // An IntQueue2 is [] if size == 0, otherwise for each additional entry in entries size = size + 1.
    // The front of the queue = front.

    // Representation Invariant for every IntQueue2 q:
    // entries != null
    // no entry may be null
    // entries are sorted based on condition, i.e:
    // If front + size < entries.length, then entries are sorted by most recently added last such that
    // for all index 0 <= i <= k, where i is the index and k is the last index in entries, i+1 was added after i.
    // Otherwise if front + size >= entries.length then entries are sorted by most recently added last such that
    // for all indices i from f <= i <= k, where f is the "front" index and k is the last index in entries, and for all
    // indices j from b <= j <= e, where b is the first index in entries and e is the most recently added entry and
    // where indices e < f, for all such indices i, i+1 was added after i, for all such indices j, j+1 was
    // added after j, and all j were added after all i.
